...
% moving adjacency matrix calculation to the receipe script to avoid multiple calculation in parallel computations  - Update IZ 11-12
% adjacencyMatrix = calculateMeshAdjacency(userOptions.nVertices,
% userOptions.sourceSearchlightRadius, userOptions);
...
for k=1:length(vertices)
    ...
    verticesCurrentlyWithinRadius = userOptions.adjacencyMatrix(vertex,:); % update adjacency matrix as a userOptions field IZ 11-12
