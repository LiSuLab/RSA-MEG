...
%
% [maskedBrains =] fMRIDataMasking(fullBrainVols, binaryMasks_nS, userOptions)
%
...
%                userOptions.analysisName_MaskedBrains.mat
%                        Contains the masked brains in a struct such that
%                        maskedBrains.(mask).(subject) is a [nMaskedVoxels
...
MaskedBrainsFilename = [userOptions.analysisName, '_MaskedBrains.mat'];
...
if overwriteFlag
	...
	for mask = 1:nMasks % For each mask...
		...
		for subject = 1:nSubjects % and for each subject...
			...
			maskedBrains.(thisMask).(thisSubject) = insideMaskVoxels;
	...
	save(MaskedBrainsFilename, 'maskedBrains');
