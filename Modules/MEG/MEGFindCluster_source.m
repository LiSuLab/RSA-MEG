...
if overwriteFlag
    ...
    if isnan(userOptions.primaryThreshold)
        ...
        if userOptions.maskingFlag
            ...
            t_values_lh.data = masking(t_values_lh.data, userOptions.indexMasks, 1:2:nMasks); % assuming oddly numbered are left hemisphere and vice versa IZ 03/12
            t_values_rh.data = masking(t_values_rh.data, userOptions.indexMasks, 2:2:nMasks);
        end
    ...
    %             %                     nMasks = size(userOptions.maskNames,2);
    %             %                     t_values_lh.data = masking(t_values_lh.data, indexMasks, 1:nMasks);
    %             %                     t_values_rh.data = masking(t_values_rh.data, indexMasks, 1:nMasks);
    %             %                 end
    %
    %             t_distribution = sort([t_values_lh.data,t_values_rh.data]);
    %             percent = 0.05;
