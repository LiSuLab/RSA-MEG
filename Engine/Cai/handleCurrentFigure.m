...
if ~isfield(userOptions, 'analysisName'), error('handleCurrentFigure:NoAnalysisName', 'analysisName must be set. See help'); end%if
if ~isfield(userOptions, 'rootPath'), error('handleCurrentFigure:NoRootPath', 'rootPath must be set. See help'); end%if
...
if userOptions.saveFiguresPDF
	exportCurrentFigAsPDF(fileName, appendFlag,userOptions);
end%if:pdf
