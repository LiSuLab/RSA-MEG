function [Y, R, E] = Isomap(D, n_fcn, n_size, options,figI); 
% modified by nk, feb 2007
...
if exist('figI','var')
    ...
    elseif numel(figI)==2
        ...
        figI_default(1:numel(figI))=figI;
    end
end
figI=figI_default;
...
if (displ==1)
    ...
    figure(figI(1,1)); subplot(figI(1,2),figI(1,3),figI(1,4));
    cla; hold on;
    plot(dims, R, 'bo');
    plot(dims, R, 'b-');
    hold off
    ...
    twod = find(dims==2);
    if ~isempty(twod)
        figure(figI(2,1)); subplot(figI(2,2),figI(2,3),figI(2,4));
        cla; hold on;
        plot(Y.coords{twod}(1,:), Y.coords{twod}(2,:), 'ko');
        if (overlay == 1)
            gplot_nk(E(Y.index, Y.index), [Y.coords{twod}(1,:); Y.coords{twod}(2,:)]');
