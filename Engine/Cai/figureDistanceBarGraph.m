...
function figureDistanceBarGraph(referenceRDMStack, testRDMs, userOptions, localOptions)
...
titleString = {titleString; [' \fontsize{9}"' userOptions.distanceMeasure '" distance from \bf' localOptions.referenceName '\rm.']};
...
bar(x, sortedDs, 'FaceColor', [.6 .6 .6], 'EdgeColor', 'none');
hold on;
if bootstrapWorked
	errorbar(x, sortedDs, sortedEs, 'Color', [0 0 0], 'LineWidth', 2, 'LineStyle', 'none');
end%if:bootstrapWorked

for test = 1:nTestRDMs
	text(test, 0, ['\bf',deunderscore(testRDMs(sortedIs(test)).name)], 'Rotation', 90, 'Color', testRDMs(sortedIs(test)).color);
end
