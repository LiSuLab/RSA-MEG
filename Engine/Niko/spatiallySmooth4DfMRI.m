function [Y,smoothedYfilename]=spatiallySmooth4DfMRI(Y_OR_Yfilename,volSize_OR_mask,gaussianKernelFWHM_vx)
...
if gaussianKernelFWHM_vx>0
    h=waitbar(0,['Smoothing current data set with Gaussian Kernel of ',num2str(gaussianKernelFWHM_vx),'-voxel FWHM...']);
    ...
    for t=1:nTime
        ...
        cSpatialVol=smooth3(cSpatialVol,'gaussian',[boxWidth_vx 1 1], gaussianKernelSD_vx);
        cSpatialVol=smooth3(cSpatialVol,'gaussian',[1 boxWidth_vx 1], gaussianKernelSD_vx);
        cSpatialVol=smooth3(cSpatialVol,'gaussian',[1 1 boxWidth_vx], gaussianKernelSD_vx);
    ...
    close(h);
    disp(['Time required for smoothing data set with Gaussian Kernel of ',num2str(gaussianKernelFWHM_vx),' voxels FWHM:']);
    toc;
end
